      ...
      <div style={{ background: "#faf9f7", minHeight: "100vh", color: "#1c1917", fontFamily: "'Inter','Segoe UI',system-ui,sans-serif" }}>
        ...
        <div style={{ maxWidth: 960, margin: "0 auto", padding: "80px 24px 80px", position: "relative", zIndex: 1 }}>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, ease: EXPO }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: "#1c1917", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 14 }}>Free Calculator · ETA & Search Fund Tool</p>
          ...
          <div className="grid-2col" style={{ gap: 20 }}>
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.55, ease: EXPO }}
              style={{ background: "#fff", border: "1px solid #d6d3d1", borderRadius: 16, padding: "28px", boxShadow: "0 1px 3px rgba(0,0,0,0.06)", display: "flex", flexDirection: "column", gap: 16 }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: "#78716c", textTransform: "uppercase", letterSpacing: "0.09em", margin: "0 0 8px" }}>Financial Inputs</p>
              <div>
                ...
                <p style={{ fontSize: 11, color: "#a8a29e", marginTop: 4 }}>SDE range: {sdeMin}× – {sdeMax}× for {sector}</p>
              </div>
              {fields.map(([label, val, set]) => (
                <div key={label}>
                  <label style={{ fontSize: 13, fontWeight: 600, color: "#44403c", display: "block", marginBottom: 6 }}>{label}</label>
              ...
              <div style={{ borderTop: "1px solid #e7e5e4", paddingTop: 16 }}>
                ...
                <p style={{ fontSize: 11, color: "#a8a29e", marginTop: 5 }}>Cost of hired GM if you don&apos;t plan to work in the business</p>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18, duration: 0.55, ease: EXPO }}
              ...
              <div style={{ background: "#fff", border: "1px solid #d6d3d1", borderRadius: 16, padding: "24px", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
                <p style={{ fontSize: 12, fontWeight: 700, color: "#78716c", textTransform: "uppercase", letterSpacing: "0.09em", marginBottom: 18 }}>Reconciliation Bridge</p>
                {[
                  { label: "Net Profit",         val: npV,                                    color: "#44403c" },
                  { label: "+ Owner Salary",      val: osV,                                   color: "#059669" },
                  { label: "+ Owner Perks",       val: opV,                                   color: "#059669" },
                  { label: "+ One-off Costs",     val: ocV,                                   color: "#059669" },
                  { label: "= SDE",               val: sde,                                   color: "#1c1917", bold: true },
                  { label: "− Replacement Mgr",   val: -rmV,                                  color: "#dc2626" },
                  { label: "= Adjusted SDE",      val: adjustedEbitda,                        color: "#292524", bold: true },
                  { label: "+ D&A + Interest",    val: depV + amoV + intV,                   color: "#d97706" },
                  { label: "= EBITDA",            val: ebitda,                                color: "#d97706", bold: true },
                ].map(r => (
                  <div key={r.label} style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: r.bold ? "2px solid #d6d3d1" : "1px solid #e7e5e4" }}>
                    <span style={{ fontSize: 13, color: r.bold ? "#44403c" : "#78716c", fontWeight: r.bold ? 700 : 400 }}>{r.label}</span>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                {[
                  { label: `SDE Valuation (${sdeMidMultiple.toFixed(1)}× mid)`, val: `£${(sdeValuation / 1000).toFixed(0)}k`, color: "#1c1917", bg: "#e7e5e4", border: "#d6d3d1", sub: `${sector} range: ${sdeMin}×–${sdeMax}×` },
                  ...
                  <div key={m.label} style={{ padding: "18px 16px", borderRadius: 14, background: m.bg, border: `1px solid ${m.border}`, textAlign: "center" }}>
                    ...
                    <p style={{ fontSize: 10, color: "#a8a29e", margin: 0 }}>{m.sub}</p>
                  </div>
                ))}
              </div>
