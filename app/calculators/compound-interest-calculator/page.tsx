...
export default function CompoundInterestPage() {
  ...
  const inputStyle: React.CSSProperties = { width: "100%", background: "#faf9f7", border: "1px solid #d6d3d1", borderRadius: 10, padding: "11px 14px", fontSize: 14, color: "#1c1917", outline: "none", fontFamily: "inherit" };
      ...
      <div style={{ background: "#faf9f7", minHeight: "100vh", color: "#1c1917", fontFamily: "'Inter','Segoe UI',system-ui,sans-serif" }}>
        ...
        <div style={{ maxWidth: 960, margin: "0 auto", padding: "80px 24px 80px", position: "relative", zIndex: 1 }}>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, ease: EXPO }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: "#1c1917", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 14 }}>Free Calculator · Acquisition ROI Modelling</p>
          ...
          <div className="grid-2col" style={{ gap: 20, alignItems: "start" }}>
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.55, ease: EXPO }}
              ...
              <p style={{ fontSize: 12, fontWeight: 700, color: "#78716c", textTransform: "uppercase", letterSpacing: "0.09em" }}>Inputs</p>
              {[
                ...
                <div key={f.label}>
                  <label style={{ fontSize: 13, fontWeight: 600, color: "#44403c", display: "block", marginBottom: 4 }}>{f.label}</label>
                  {f.hint && <p style={{ fontSize: 11, color: "#a8a29e", margin: "0 0 6px" }}>{f.hint}</p>}
                  <div style={{ position: "relative" }}>
                    {f.pound && <span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: "#a8a29e", fontSize: 14, fontWeight: 600 }}>£</span>}
              ...
              {/* Summary metrics */}
              <div style={{ borderTop: "1px solid #e7e5e4", paddingTop: 20, display: "flex", flexDirection: "column", gap: 10 }}>
                {[
                  { label: "Total Distributions", val: `£${totalDist.toLocaleString()}`,                           color: "#292524" },
                  { label: "Final Equity Value",  val: `£${Math.round(finalRow?.value ?? 0).toLocaleString()}`,    color: "#1c1917" },
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18, duration: 0.55, ease: EXPO }}
              ...
              <div style={{ background: "#fff", border: "1px solid #d6d3d1", borderRadius: 16, padding: "24px", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
                <p style={{ fontSize: 12, fontWeight: 700, color: "#78716c", textTransform: "uppercase", letterSpacing: "0.09em", marginBottom: 20 }}>Total Value Over Time</p>
                  ...
                  {[{ year: 0, total: initialVal }, ...schedule].map((r, i) => {
                    ...
                    return (
                      <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                        <span style={{ fontSize: 10, color: "#78716c" }}>£{r.total >= 1000000 ? (r.total / 1000000).toFixed(1) + "m" : Math.round(r.total / 1000) + "k"}</span>
                        <div style={{ width: "100%", height: `${heightPct}%`, background: i === 0 ? "#d6d3d1" : `linear-gradient(180deg, #1c1917, #292524)`, borderRadius: "4px 4px 0 0", minHeight: 4, transition: "height 0.3s" }} />
                        <span style={{ fontSize: 10, color: "#a8a29e" }}>Y{r.year}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              {/* Year table */}
              <div style={{ background: "#fff", border: "1px solid #d6d3d1", borderRadius: 16, overflow: "hidden", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
                <div style={{ display: "grid", gridTemplateColumns: "60px 1fr 1fr 1fr 1fr", padding: "12px 20px", borderBottom: "1px solid #e7e5e4", background: "#faf9f7" }}>
                  {["Year", "Growth", "Distribution", "Equity Value", "Total Value"].map(h => (
                    <span key={h} style={{ fontSize: 11, color: "#78716c", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em" }}>{h}</span>
                  ))}
                </div>
                {schedule.map((r, i) => (
                  <div key={r.year} style={{
                    ...
                    padding: "12px 20px",
                    borderBottom: i < schedule.length - 1 ? "1px solid #e7e5e4" : "none",
                    background: i % 2 === 0 ? "transparent" : "#fafafa",
                  }}>
                    <span style={{ fontSize: 13, color: "#1c1917", fontWeight: 700 }}>Y{r.year}</span>
                    <span style={{ fontSize: 13, color: "#059669" }}>+£{Math.round(r.growth).toLocaleString()}</span>
                    <span style={{ fontSize: 13, color: "#292524" }}>£{r.distribution.toLocaleString()}</span>
                    <span style={{ fontSize: 13, color: "#78716c" }}>£{Math.round(r.value).toLocaleString()}</span>
                    <span style={{ fontSize: 13, color: "#1c1917", fontWeight: 600 }}>£{Math.round(r.total).toLocaleString()}</span>
                  </div>
                ))}
              </div>
