...
export default function StampDutyPage() {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    borderRadius: 10, padding: "11px 14px 11px 28px", fontSize: 14, color: "#1c1917", outline: "none", fontFamily: "inherit",
  };
      ...
      <div style={{ background: "#faf9f7", minHeight: "100vh", color: "#1c1917", fontFamily: "'Inter','Segoe UI',system-ui,sans-serif" }}>
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "80px 24px 80px", position: "relative", zIndex: 1 }}>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, ease: EXPO }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: "#1c1917", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 14 }}>Free Calculator · 2025 UK Tax Rates</p>
            ...
            <p style={{ fontSize: 15, color: "#78716c", maxWidth: 620, lineHeight: 1.75, marginBottom: 24 }}>
          ...
          <div className="grid-2col" style={{ gap: 20, alignItems: "start" }}>
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.55, ease: EXPO }}
              style={{ background: "#fff", border: "1px solid #d6d3d1", borderRadius: 16, padding: "28px", boxShadow: "0 1px 3px rgba(0,0,0,0.06)", display: "flex", flexDirection: "column", gap: 18 }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: "#78716c", textTransform: "uppercase", letterSpacing: "0.09em" }}>Deal Parameters</p>

              {/* Deal type toggle */}
              <div>
                <label style={{ fontSize: 13, fontWeight: 600, color: "#44403c", display: "block", marginBottom: 8 }}>Deal Structure</label>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  {(["assets", "shares"] as const).map(t => (
                    <button key={t} onClick={() => setDealType(t)}
                      style={{ padding: "10px 14px", borderRadius: 10, border: `1px solid ${dealType === t ? "#d6d3d1" : "#d6d3d1"}`, background: dealType === t ? "#e7e5e4" : "#faf9f7", color: dealType === t ? "#1c1917" : "#78716c", fontSize: 13, fontWeight: 600, cursor: "pointer", textTransform: "capitalize" }}>
              ...
              {/* Total price */}
              <div>
                <label style={{ fontSize: 13, fontWeight: 600, color: "#44403c", display: "block", marginBottom: 6 }}>Total Purchase Price</label>
              ...
              {dealType === "assets" && (
                <>
                  <div style={{ padding: "16px", borderRadius: 12, background: "#faf9f7", border: "1px solid #d6d3d1" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                      <label style={{ fontSize: 13, fontWeight: 600, color: "#44403c" }}>Includes Property / Lease</label>
                      <button onClick={() => setIncludesProperty(!includesProperty)}
                        style={{ width: 40, height: 22, borderRadius: 11, border: "none", background: includesProperty ? "#1c1917" : "#d6d3d1", cursor: "pointer", position: "relative", transition: "background 0.15s" }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                          {(["commercial", "residential"] as const).map(t => (
                            <button key={t} onClick={() => setIsResidential(t === "residential")}
                              style={{ padding: "8px 10px", borderRadius: 8, border: `1px solid ${(!isResidential && t === "commercial") || (isResidential && t === "residential") ? "#d6d3d1" : "#d6d3d1"}`, background: (!isResidential && t === "commercial") || (isResidential && t === "residential") ? "#e7e5e4" : "#faf9f7", color: (!isResidential && t === "commercial") || (isResidential && t === "residential") ? "#1c1917" : "#78716c", fontSize: 12, fontWeight: 600, cursor: "pointer", textTransform: "capitalize" }}>
                              {t}
                            </button>
                          ))}
                        </div>
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18, duration: 0.55, ease: EXPO }}
              ...
              <div style={{ background: "linear-gradient(135deg,#e7e5e4,#e7e5e4)", border: "1px solid #d6d3d1", borderRadius: 16, padding: "28px", textAlign: "center" }}>
                <p style={{ fontSize: 11, fontWeight: 700, color: "#1c1917", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 8 }}>Total Tax Liability</p>
                <p style={{ fontSize: 52, fontWeight: 800, letterSpacing: "-0.05em", color: "#1c1917", margin: "0 0 4px", lineHeight: 1 }}>
                  £{totalDuty.toLocaleString()}
                </p>
                <p style={{ fontSize: 13, color: "#78716c" }}>Effective rate: {effectiveRate.toFixed(2)}%</p>
              </div>
              ...
              <div style={{ background: "#fff", border: "1px solid #d6d3d1", borderRadius: 16, padding: "24px", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
                ...
                {[
                  { label: "Purchase Price", val: `£${priceVal.toLocaleString()}`, color: "#44403c" },
                  ...
                  { label: "Total Tax Due", val: `£${totalDuty.toLocaleString()}`, color: "#1c1917", bold: true },
                  { label: "All-in Acquisition Cost", val: `£${totalCost.toLocaleString()}`, color: "#1c1917", bold: true },
                ].map(r => (
                  <div key={r.label} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #e7e5e4" }}>
                    ...
                    <span style={{ fontSize: 13, fontWeight: (r as { bold?: boolean }).bold ? 700 : 600, color: (r as { color?: string }).color ?? "#44403c" }}>{r.val}</span>
                  </div>
                ))}
              </div>
          ...
          <div style={{ marginTop: 40, borderTop: "1px solid #d6d3d1", paddingTop: 28, display: "flex", gap: 10, flexWrap: "wrap" }}>
            {["/calculators/dscr-calculator", "/calculators/sme-debt-capacity", "/calculators/sde-to-ebitda", "/calculators/compound-interest-calculator"].map(h => (
              <Link key={h} href={h} style={{ padding: "8px 16px", borderRadius: 9999, background: "#fff", border: "1px solid #d6d3d1", color: "#475569", fontSize: 13, textDecoration: "none" }}>
