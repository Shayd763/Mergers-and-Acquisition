...
function ArbitrageCalc() {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    borderRadius: 10, padding: "10px 14px", fontSize: 14, color: "#1c1917",
    outline: "none", fontFamily: "inherit",
  };

  return (
    <div style={{ background: "#fff", border: "1px solid #d6d3d1", borderRadius: 20, overflow: "hidden", boxShadow: "0 4px 24px rgba(0,0,0,0.07)" }}>
      <div style={{ padding: "10px 18px", borderBottom: "1px solid #e7e5e4", display: "flex", alignItems: "center", gap: 8, background: "#faf9f7" }}>
        ...
        <span style={{ fontSize: 11, color: "#a8a29e", fontWeight: 500, marginLeft: 4 }}>Deal Arbitrage Calculator</span>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0 }}>
        {/* Inputs */}
        <div style={{ padding: "28px 24px", borderRight: "1px solid #e7e5e4" }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: "#78716c", textTransform: "uppercase", letterSpacing: "0.09em", marginBottom: 20 }}>Deal Inputs</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {[
              ...
              <div key={f.label}>
                <label style={{ fontSize: 12, fontWeight: 600, color: "#44403c", display: "block", marginBottom: 5 }}>{f.label}</label>
                <div style={{ position: "relative" }}>
                  <span style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", color: "#a8a29e", fontSize: 12, fontWeight: 600 }}>{f.prefix}</span>
        ...
        <div style={{ padding: "28px 24px" }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: "#78716c", textTransform: "uppercase", letterSpacing: "0.09em", marginBottom: 20 }}>Deal Outputs</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[
              { label: "Asking Price", val: `£${(askingPrice / 1000).toFixed(0)}k`, color: "#1c1917" },
              { label: "Bank Debt (60%)", val: `£${(bankLoan / 1000).toFixed(0)}k`, color: "#1c1917" },
              { label: "Vendor Finance (20%)", val: `£${(vendorFinance / 1000).toFixed(0)}k`, color: "#292524" },
              ...
              <div key={r.label} style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: "1px solid #e7e5e4" }}>
          ...
          <div style={{ marginTop: 16, borderTop: "2px solid #d6d3d1", paddingTop: 16 }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: "#78716c", textTransform: "uppercase", letterSpacing: "0.09em", marginBottom: 12 }}>Your Arbitrage Take</p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {[
                { label: "Day 1 Fee", val: `£${sourcingFee.toLocaleString()}`, color: "#d97706" },
                { label: "Your Equity Stake", val: `£${(equityValue / 1000).toFixed(0)}k`, color: "#292524" },
                { label: "Your Capital In", val: "£0", color: "#059669" },
                { label: "Est. 5yr IRR", val: irr > 0 ? `${(irr * 100).toFixed(0)}%` : "—", color: "#1c1917" },
              ].map(m => (
                <div key={m.label} style={{ background: "#faf9f7", border: "1px solid #d6d3d1", borderRadius: 10, padding: "10px 12px", textAlign: "center" }}>
                  <p style={{ fontSize: 9, color: "#a8a29e", margin: "0 0 4px", textTransform: "uppercase", letterSpacing: "0.09em" }}>{m.label}</p>
          ...
          <Link href="/dashboard/triage" style={{ textDecoration: "none", display: "block", marginTop: 16 }}>
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
              style={{ width: "100%", padding: "11px", borderRadius: 10, border: "none", cursor: "pointer", background: "linear-gradient(135deg,#1c1917,#292524)", color: "#fff", fontSize: 13, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}>
  ...
  return (
    <div style={{ background: bg, border: `1px solid ${border}`, borderRadius: 14, padding: "18px 20px", display: "flex", gap: 14 }}>
      ...
      <div>
        ...
        <p style={{ fontSize: 13, color: "#78716c", margin: 0, lineHeight: 1.65 }}>{body}</p>
      </div>
    </div>
  );
}
...
function SectionHead({ step, title, sub }: { step: string; title: string; sub: string }) {
  return (
    <div style={{ marginBottom: 32 }}>
      <span style={{ fontSize: 11, fontWeight: 800, color: "#1c1917", letterSpacing: "0.12em", textTransform: "uppercase" }}>{step}</span>
      <h2 style={{ fontSize: "clamp(24px, 3.5vw, 38px)", fontWeight: 800, letterSpacing: "-0.04em", color: "#1c1917", margin: "8px 0 12px", lineHeight: 1.15 }}>{title}</h2>
      <p style={{ fontSize: 15, color: "#78716c", lineHeight: 1.7, margin: 0, maxWidth: 600 }}>{sub}</p>
    </div>
  );
}
  ...
  return (
    <div style={{ borderBottom: "1px solid #e7e5e4" }}>
      <button onClick={() => setOpen(!open)} style={{ width: "100%", background: "none", border: "none", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "18px 0", textAlign: "left" }}>
        <span style={{ fontSize: 14, fontWeight: 600, color: "#1c1917" }}>{q}</span>
      ...
      <AnimatePresence initial={false}>
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.22, ease: EXPO }} style={{ overflow: "hidden" }}>
            <p style={{ fontSize: 13, color: "#78716c", lineHeight: 1.75, paddingBottom: 16, margin: 0 }}>{a}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
export default function DealSourcingGuidePage() {
  ...
  const STEPS = [
    {
      n: "01", icon: <Target size={22} />, accent: "#1c1917",
      ...
      content: (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <p style={{ fontSize: 14, color: "#78716c", lineHeight: 1.75 }}>
            Target businesses with <strong style={{ color: "#1c1917" }}>£150k–£400k SDE</strong> in sectors with recurring demand: engineering consultancies, accounting practices, day nurseries, logistics businesses, and specialist manufacturing. These sectors command 3.0–4.2× SDE multiples and have predictable, recurring revenue that banks love.
          </p>
          <p style={{ fontSize: 14, color: "#78716c", lineHeight: 1.75 }}>
            The ideal seller is a 60+ founder who built the business over 20 years and wants a clean exit. They typically have <strong style={{ color: "#1c1917" }}>no interest in a protracted sale process</strong> — and will seriously consider vendor finance if it gets a deal done at their target price.
          </p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[
              ...
              <div key={c.title} style={{ background: "#faf9f7", border: "1px solid #d6d3d1", borderRadius: 12, padding: "16px 18px" }}>
                <p style={{ fontSize: 11, fontWeight: 700, color: "#1c1917", textTransform: "uppercase", letterSpacing: "0.09em", margin: "0 0 12px" }}>{c.title}</p>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {c.items.map(i => (
                    <div key={i} style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      <CheckCircle size={11} color="#059669" />
                      <span style={{ fontSize: 13, color: "#78716c" }}>{i}</span>
                    </div>
                  ))}
                </div>
              </div>
    ...
    {
      n: "02", icon: <BarChart3 size={22} />, accent: "#292524",
      ...
      content: (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ background: "#1c1917", borderRadius: 16, padding: "28px" }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: "#a8a29e", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 20 }}>Standard Arbitrage Capital Stack — £600k Deal Example</p>
              ...
              {[
                ...
                <div key={s.label} style={{ display: "flex", gap: 16, padding: "14px 16px", borderRadius: 12, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ fontSize: 11, color: "#a8a29e", margin: "0 0 3px" }}>{s.rate}</p>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
            {[
              ...
              <div key={p.page} style={{ background: "#faf9f7", border: "1px solid #d6d3d1", borderRadius: 12, padding: "16px 16px" }}>
                <p style={{ fontSize: 10, fontWeight: 700, color: "#059669", textTransform: "uppercase", letterSpacing: "0.09em", margin: "0 0 4px" }}>{p.page}</p>
                <p style={{ fontSize: 13, fontWeight: 700, color: "#1c1917", margin: "0 0 12px" }}>{p.title}</p>
                {p.items.map(i => (
                  <div key={i} style={{ display: "flex", gap: 7, marginBottom: 6 }}>
                    <CheckCircle size={11} color="#059669" style={{ flexShrink: 0, marginTop: 2 }} />
                    <span style={{ fontSize: 12, color: "#78716c" }}>{i}</span>
                  </div>
                ))}
              </div>
            ))}
          </div>
          <Callout icon={<Zap size={15} />} title="Why the Credit Memo is your unfair advantage" color="#1c1917" bg="#e7e5e4" border="#d6d3d1"
    ...
    },
    {
      ...
      content: (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ background: "#fff", border: "1px solid #d6d3d1", borderRadius: 16, padding: "24px" }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: "#78716c", textTransform: "uppercase", letterSpacing: "0.09em", marginBottom: 16 }}>The Investor Pitch — Verbatim Script</p>
            <div style={{ background: "#faf9f7", borderRadius: 12, padding: "20px 22px", borderLeft: "3px solid #d97706" }}>
              <p style={{ fontSize: 14, color: "#44403c", lineHeight: 1.8, margin: 0, fontStyle: "italic" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div style={{ background: "#faf9f7", border: "1px solid #d6d3d1", borderRadius: 12, padding: "16px 18px" }}>
      ...
      content: (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <div style={{ background: "#fff", border: "1px solid #d6d3d1", borderRadius: 14, padding: "22px" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {[
                  ...
                  <div key={r.label} style={{ padding: "11px 14px", background: "#faf9f7", borderRadius: 10, border: "1px solid #d6d3d1" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                      <span style={{ fontSize: 12, color: "#78716c" }}>{r.label}</span>
                      <span style={{ fontSize: 14, fontWeight: 800, color: "#1c1917", letterSpacing: "-0.02em" }}>{r.val}</span>
                    </div>
                    <p style={{ fontSize: 11, color: "#a8a29e", margin: 0, fontStyle: "italic" }}>{r.note}</p>
                  </div>
                ))}
              </div>
            </div>
            <div style={{ background: "#fff", border: "1px solid #d6d3d1", borderRadius: 14, padding: "22px" }}>
              <p style={{ fontSize: 11, fontWeight: 700, color: "#1c1917", textTransform: "uppercase", letterSpacing: "0.09em", margin: "0 0 16px" }}>5-Year Hold — Ongoing</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {[
                  ...
                  { label: "Target 5yr equity IRR", val: "35–45%", color: "#1c1917", note: "On investors' capital (they brought the risk, not you)" },
                ].map(r => (
                  <div key={r.label} style={{ padding: "11px 14px", background: "#faf9f7", borderRadius: 10, border: "1px solid #d6d3d1" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                      <span style={{ fontSize: 12, color: "#78716c" }}>{r.label}</span>
                      <span style={{ fontSize: 14, fontWeight: 800, color: (r as { color?: string }).color ?? "#1c1917", letterSpacing: "-0.02em" }}>{r.val}</span>
                    </div>
                    <p style={{ fontSize: 11, color: "#a8a29e", margin: 0, fontStyle: "italic" }}>{r.note}</p>
                  </div>
                ))}
          ...
          <Callout icon={<Shield size={15} />} title="Legal note" color="#78716c" bg="#faf9f7" border="#d6d3d1"
  ...
  return (
    <div style={{ background: "#faf9f7", minHeight: "100vh", color: "#1c1917", fontFamily: "'Inter','Segoe UI',system-ui,sans-serif" }}>
      <div style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none", backgroundImage: "linear-gradient(rgba(148,163,184,0.12) 1px,transparent 1px),linear-gradient(90deg,rgba(148,163,184,0.12) 1px,transparent 1px)", backgroundSize: "64px 64px", maskImage: "radial-gradient(ellipse 80% 50% at 50% 0%,transparent 20%,#faf9f7 75%)", WebkitMaskImage: "radial-gradient(ellipse 80% 50% at 50% 0%,transparent 20%,#faf9f7 75%)" }} />
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "100px 24px 80px", position: "relative", zIndex: 1 }}>
        ...
        <motion.div variants={stagger} initial="hidden" animate="visible" style={{ marginBottom: 72 }}>
          ...
          <motion.h1 variants={fadeUp} style={{ fontSize: "clamp(32px,5vw,60px)", fontWeight: 800, letterSpacing: "-0.04em", margin: "0 0 18px", lineHeight: 1.1 }}>
            ...
            <span style={{ background: "linear-gradient(135deg,#1c1917,#292524)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>With Zero Personal Capital.</span>
          </motion.h1>
          <motion.p variants={fadeUp} style={{ fontSize: 17, color: "#78716c", maxWidth: 640, lineHeight: 1.75, marginBottom: 32 }}>
          ...
          <motion.div variants={fadeUp} style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
            {["Source → Structure → Prove → Capitalise → Reap"].map(s => (
              <div key={s} style={{ background: "#1c1917", color: "#f4f4f5", padding: "10px 20px", borderRadius: 9999, fontSize: 13, fontWeight: 700, letterSpacing: "0.02em", fontFamily: "monospace" }}>{s}</div>
            ))}
            <Link href="/dashboard/triage" style={{ textDecoration: "none" }}>
              <motion.button whileHover={{ scale: 1.03 }} style={{ padding: "10px 20px", borderRadius: 9999, border: "1px solid #d6d3d1", background: "#fff", color: "#1c1917", fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 7 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease: EXPO }} style={{ marginBottom: 80 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
            <div style={{ width: 2, height: 20, background: "linear-gradient(180deg,#1c1917,#292524)", borderRadius: 2 }} />
            <p style={{ fontSize: 13, fontWeight: 700, color: "#78716c", textTransform: "uppercase", letterSpacing: "0.09em", margin: 0 }}>Live Arbitrage Calculator — Adjust the inputs to model your deal</p>
          </div>
          <ArbitrageCalc />
        </motion.div>
            ...
            <motion.div key={s.n} initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.6, ease: EXPO }}>
              <div style={{ display: "flex", gap: 24, alignItems: "flex-start" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0, paddingTop: 4 }}>
                  ...
                  {idx < STEPS.length - 1 && <div style={{ width: 2, flex: 1, minHeight: 40, background: "linear-gradient(180deg,#d6d3d1,transparent)", marginTop: 12 }} />}
                </div>
                {/* Content */}
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease: EXPO }} style={{ marginTop: 80 }}>
          <h2 style={{ fontSize: "clamp(24px,3.5vw,36px)", fontWeight: 800, letterSpacing: "-0.04em", color: "#1c1917", margin: "0 0 32px" }}>Common questions on deal arbitrage.</h2>
          <div style={{ background: "#fff", border: "1px solid #d6d3d1", borderRadius: 20, padding: "0 28px", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
            {[
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease: EXPO }} style={{ marginTop: 72 }}>
          <div style={{ background: "linear-gradient(135deg,#1c1917,#1e293b)", borderRadius: 24, padding: "52px 44px", textAlign: "center", position: "relative", overflow: "hidden" }}>
            ...
            <p style={{ fontSize: 12, fontWeight: 700, color: "#a8a29e", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 16, position: "relative" }}>Ready to start?</p>
            ...
            <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap", position: "relative" }}>
              <Link href="/dashboard/triage" style={{ textDecoration: "none" }}>
                <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
                  style={{ padding: "14px 28px", borderRadius: 12, border: "none", cursor: "pointer", background: "linear-gradient(135deg,#1c1917,#a855f7)", color: "#fff", fontSize: 15, fontWeight: 700, display: "flex", alignItems: "center", gap: 8, boxShadow: "0 0 40px rgba(99,102,241,0.4)" }}>
